...
INPUT_LOG = "spike_featuremaps.txt"
OUT_TXT = "formatted_featuremaps.txt"
...
for raw in lines:
    ...
    if current is not None and line.startswith("[TB]"):
        bytes_now = parse_tb_line(line)

        # Convert only OP0 input feature map from raw uint8-style image bytes
        # to signed-int8 raw hex bytes.
        #
        # Example:
        # 0B -> 8B
        # 10 -> 90
        # 12 -> 92
        # 1D -> 9D
        if current["kind"] == "IA" and current["op"] == 0:
            bytes_now = [
                f"{((int(b, 16) - 128) & 0xFF):02X}"
                for b in bytes_now
            ]

        current["data"].extend(bytes_now)
        continue
