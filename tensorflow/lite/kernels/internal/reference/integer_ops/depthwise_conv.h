...
inline void DepthwiseConvPerChannel(
    ...
    const int32_t* bias_data, const RuntimeShape& output_shape,
    int8_t* output_data) {
  // Counts DepthwiseConvPerChannel calls.
  // Call 0 = OP00
  // Call 1 = OP01  <-- first bad layer
      ...
      for (int out_x = 0; out_x < output_width; ++out_x) {
        for (int in_channel = 0; in_channel < input_depth; ++in_channel) {
          for (int m = 0; m < depth_multiplier; ++m) {
            ...
            // Only debug OP01, first output pixel, channel 1.
            // OP01 byte 1 is the first mismatch:
            // FPGA = 0x85, Spike = 0x83.
            const bool dump_dw =
                (this_dw_call == 1 &&
                 batch == 0 &&
                 out_y == 0 &&
            ...
            for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
              for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
                ...
                if (is_point_inside_image) {
                  ...
                  const int32_t input_val = input_data[input_offset_index];
                  const int32_t filter_val = filter_data[filter_offset_index];
                  const int32_t input_plus_offset = input_val + input_offset;
                  const int32_t product = filter_val * input_plus_offset;
            ...
            const int8_t out_value = static_cast<int8_t>(acc);

            if (dump_dw) {
              RawPutcD('['); RawPutcD('D'); RawPutcD('9'); RawPutcD(']');
              RawTagHexD('O', static_cast<uint32_t>(out_offset));
              RawTagHexD('p', static_cast<uint32_t>(
                                  reinterpret_cast<uintptr_t>(
                                      &output_data[out_offset])));
              RawTagHexD('v', static_cast<uint32_t>(
                                  static_cast<uint8_t>(out_value)));
              RawNewlineD();
            }

            output_data[out_offset] = out_value;
          }
        }
      }
    }
