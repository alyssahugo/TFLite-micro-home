...
//
// Input / weight / bias SPADs are NOT AXI-mapped.
// Therefore their "base addresses" below are LOCAL SPAD offsets,
// not CPU/AXI addresses.
//
// top.sv should route DMA WDATA into the selected SPAD using M_SPAD_SEL.
// The DMA DST address is used as the local write address/offset.
//
...
#define CONV_SPAD_M_LOCAL_BASE    0x00000000u
#define CONV_SPAD_S_LOCAL_BASE    0x00000000u

// FILL RTL DETAIL HERE:
// This one must be a real AXI/CPU-readable address.
#define CONV_SPAD_O_AXI_BASE      0x40060000u

// ================================================================
// CSR offsets from systolic_csr.v
// ================================================================
...
// ================================================================
// FILL RTL DETAIL HERE: DMA beat size
//
// Your DMA parameter DATA_WIDTH defaults to 32, so BPB = 4.
// If your instantiated DMA DATA_WIDTH is 64, change this to 8.
// ================================================================
...
// CSR has no bias address register, so this assumes fixed bias SPAD read.
...
// ================================================================
// Output AXI SPAD to TFLM output_data
//
// Output SPAD should be AXI/CPU-readable
// Accelerator output is already final int8 for one output channel
// Output SPAD layout assumed:
//   spad_out[out_y * output_w + out_x]
// ================================================================

static inline bool ConvAccelStoreOneChannelFromOutputAxiSpadToOutputData(
    const RuntimeShape& output_shape,
    int batch,
    int output_channel,
    int output_h,
...
static inline bool TryConvAccelPointwisePerChannel(
  ...
  for (int batch = 0; batch < batches; ++batch) {
    ...
    for (int oc = 0; oc < output_c; ++oc) {
      ...
      // Destination is LOCAL weight SPAD offset, not AXI address.
      ...
      // top.sv/systolic reads bias from fixed bias SPAD local offset 0.
      if (bias_data) {
      ...
      if (!ConvAccelStoreOneChannelFromOutputAxiSpadToOutputData(
              output_shape,
              batch,
              oc,
              output_h,
