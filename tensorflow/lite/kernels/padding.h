...
inline int DivByStride1Or2_NoDiv(int numerator, int stride) {
  // This project/model has only stride 1 or stride 2.
  // For normal TFLM output-size math, numerator should be nonnegative.
  if (stride == 1) {
    return numerator;
  }
  ...
  // Debug trap/fallback for unexpected stride.
  // Do NOT use numerator / stride here, because that can reintroduce DIV.
  return 0;
}
...
inline int ComputePadding(int stride, int dilation_rate, int in_size,
  ...
  // Original was total_padding / 2.
  // total_padding is nonnegative here, so shift is safe for /2.
  return total_padding >> 1;
}
...
inline int ComputePaddingWithOffset(int stride, int dilation_rate, int in_size,
  ...
  //   return total_padding / 2;
  //
  // Since total_padding is nonnegative:
  //   x % 2 == x & 1
  //   x / 2 == x >> 1
  *offset = total_padding & 1;
  return total_padding >> 1;
}
...
inline int ComputeOutSize(TfLitePadding padding, int image_size,
  ...
  switch (padding) {
    case kTfLitePaddingSame:
      // Original: (image_size + stride - 1) / stride
      numerator = image_size + stride - 1;
      return DivByStride1Or2_NoDiv(numerator, stride);

    case kTfLitePaddingValid:
      // Original: (image_size + stride - effective_filter_size) / stride
      numerator = image_size + stride - effective_filter_size;
      return DivByStride1Or2_NoDiv(numerator, stride);
