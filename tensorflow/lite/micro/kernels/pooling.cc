...
TfLiteStatus AverageEval(TfLiteContext* context, TfLiteNode* node) {
  // RawPutc('['); RawPutc('A'); RawPutc('0'); RawPutc(']');
  // RawNewline();
  ...
  const OpDataPooling* data =
      static_cast<const OpDataPooling*>(node->user_data);
  // RawPutc('['); RawPutc('A'); RawPutc('1'); RawPutc(']');
  // RawNewline();
  const TfLiteEvalTensor* input =
      micro::GetEvalInput(context, node, kPoolingInputTensor);
  // RawPutc('['); RawPutc('A'); RawPutc('2'); RawPutc(']');
  // RawNewline();
  TfLiteEvalTensor* output =
      micro::GetEvalOutput(context, node, kPoolingOutputTensor);
  // RawPutc('['); RawPutc('A'); RawPutc('3'); RawPutc(']');
  // RawNewline();
  // Inputs and outputs share the same type, guaranteed by the converter.
  // RawPutc('['); RawPutc('A'); RawPutc('T'); RawPutc(']');
  // RawTagHex('t', static_cast<uint32_t>(input->type));
  // RawNewline();
  switch (input->type) {
    case kTfLiteFloat32:
      // RawPutc('['); RawPutc('A'); RawPutc('5'); RawPutc(']');
      // RawNewline(); 
      AveragePoolingEvalFloat(context, node, params, data, input, output);
      // RawPutc('['); RawPutc('A'); RawPutc('7'); RawPutc(']');
      // RawNewline(); 
     
      break;
    case kTfLiteInt8:
      // RawPutc('['); RawPutc('A'); RawPutc('4'); RawPutc(']');
      // RawNewline();
      AveragePoolingEvalQuantized<int8_t>(context, node, params, data, input,
                                          output);
      // RawPutc('Y'); RawPutc('A'); RawPutc('Y'); RawPutc('!');
      // RawNewline();
      break;
    case kTfLiteInt16:
      // RawPutc('['); RawPutc('A'); RawPutc('6'); RawPutc(']');
      // RawNewline();
      AveragePoolingEvalQuantized<int16_t>(context, node, params, data, input,
                                           output);
      break;
