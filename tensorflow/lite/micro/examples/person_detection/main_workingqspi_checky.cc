...
static constexpr uint32_t SR_RX_FULL        = (1u << 1);
static constexpr uint32_t SR_TX_EMPTY       = (1u << 2);
static constexpr uint32_t SR_TX_FULL        = (1u << 3);
static constexpr uint32_t SR_MODF           = (1u << 4);
static constexpr uint32_t SR_SLAVE_SEL      = (1u << 5);
...
// keep same for now to confirm behavior
...
static void print_spisr(const char* tag) {
  uint32_t sr = qspi_read32(QSPI_BASE + REG_SPISR);
  uart_puts(tag);
  uart_puts(" SPISR=0x");
  uart_puthex32(sr);
  uart_puts(" [");
  uart_putc((sr & SR_RX_EMPTY)       ? 'R' : '-');
  uart_putc((sr & SR_RX_FULL)        ? 'r' : '-');
  uart_putc((sr & SR_TX_EMPTY)       ? 'T' : '-');
  uart_putc((sr & SR_TX_FULL)        ? 't' : '-');
  uart_putc((sr & SR_MODF)           ? 'M' : '-');
  uart_putc((sr & SR_SLAVE_SEL)      ? 'S' : '-');
  uart_putc((sr & SR_CPOL_CPHA_ERR)  ? 'C' : '-');
  uart_putc((sr & SR_SLAVE_MODE_ERR) ? 'E' : '-');
  uart_putc((sr & SR_MSB_ERR)        ? 'B' : '-');
  uart_putc((sr & SR_LOOPBACK_ERR)   ? 'L' : '-');
  uart_putc((sr & SR_COMMAND_ERR)    ? 'X' : '-');
  uart_puts("]");
  uart_newline();
}
...
int main() {
  ...
  // Only try to drain if TX phase completed
  if (tx_ok) {
    uart_puts("Draining RX:\r\n");
    for (int i = 0; i < 6; ++i) {
      int timeout = 1000;
      while ((qspi_read32(QSPI_BASE + REG_SPISR) & SR_RX_EMPTY) && --timeout) { }
      if (!timeout) {
        uart_puts("RX timeout at i=");
        uart_puthex8((uint8_t)i);
        uart_newline();
        break;
      }
      uint8_t b = qspi_pop_byte();
      uart_puts("RX[");
      uart_puthex8((uint8_t)i);
      uart_puts("]=0x");
      uart_puthex8(b);
