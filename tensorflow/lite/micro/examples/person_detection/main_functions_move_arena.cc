...
//   volatile uint8_t* p = reinterpret_cast<volatile uint8_t*>(&resolver);
//   p[0] = 0;
//   RawPutc('r');
...
void setup() {
  ...
  // Optional: clear the arena manually now that it is no longer in .bss
...
// static MiniReg g_reg; 

// void setup() {
//   // TEMP: replace everything with this test
//   RawPutc('X');
//   RawPutc(' ');

//   MiniReg s = MakeMiniReg();
//   RawPutHex32((uint32_t)(uintptr_t)s.a);
//   RawPutHex32((uint32_t)(uintptr_t)s.b);
//   RawPutHex32((uint32_t)(uintptr_t)s.c);
...
//   g_reg = MakeMiniReg();
//   RawPutHex32((uint32_t)(uintptr_t)g_reg.a);
//   RawPutHex32((uint32_t)(uintptr_t)g_reg.b);
//   RawPutHex32((uint32_t)(uintptr_t)g_reg.c);
...
// // }
  ...
  // // Print pointer to g_person_detect_model_data
  // RawPutc('<');
  // uintptr_t p_addr = reinterpret_cast<uintptr_t>(g_person_detect_model_data);
  // for (int i = 7; i >= 0; --i) {
  //   RawPutHex32(p_addr);
  // }
  // RawPutc('>');

  // // Dump first 4 words directly from 0x80000110
  // // Expected:
  // // 1C000000 54464C33 00000000 00000E00
  // RawPutc('W');
  // RawPutc('[');
  // volatile uint32_t* p32 =
  //     reinterpret_cast<volatile uint32_t*>(0x80000110u);
  // for (int i = 0; i < 4; i++) {
  //   uint32_t w = p32[i];
  //   for (int n = 7; n >= 0; --n) {
  //     RawPutc("0123456789ABCDEF"[(w >> (n * 4)) & 0xF]);
  //   }
  //   if (i != 3) RawPutc('_');
  // }
  // RawPutc(']');

    // // Dump first 16 bytes using byte loads
  // // Expected:
  // // 1C00000054464C330000000000000E00
  // RawPutc('b');
  // RawPutc('[');
  // for (int i = 0; i < 16; i++) {
  //   uint8_t b = g_person_detect_model_data[i];
  //   RawPutc("0123456789ABCDEF"[b >> 4]);
  //   RawPutc("0123456789ABCDEF"[b & 0xF]);
  // }
  // RawPutc(']');

  // int v = model->version();
  // RawPutc('{');
  // RawPutc("0123456789ABCDEF"[(v >> 4) & 0xF]);
  // RawPutc("0123456789ABCDEF"[v & 0xF]);
  // RawPutc('}');